...
type Step = "email" | "code" | "paying" | "error";

export function CheckoutDialog({ onClose }: { onClose: () => void }) {
  const { email: signedInEmail, begin, confirm } = useAuth();
  const { lines } = useCart();
  const [step, setStep] = useState<Step>(signedInEmail ? "paying" : "email");
  const [email, setEmail] = useState(signedInEmail ?? "");
  const [code, setCode] = useState("");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Once we have a session, create the order and redirect to Stripe.
  useEffect(() => {
    if (step !== "paying") return;
    let cancelled = false;
    (async () => {
      try {
        const order = await api.createOrder(lines);
